...
export class UserController {
  ...
  async addOutletAccess(req: AuthRequest, res: Response) {
    try {
      const { userId, outletId } = req.body

      const access = await this.userService.addUserOutletAccess(userId, outletId)

      res.status(201).json({
        success: true,
        data: access,
        message: 'Outlet access granted'
      })
    } catch (error: any) {
      res.status(400).json({ error: error.message })
    }
  ...
  async removeOutletAccess(req: AuthRequest, res: Response) {
    try {
      const { userId, outletId } = req.params

      await this.userService.removeUserOutletAccess(userId, outletId)

      res.json({
        success: true,
        message: 'Outlet access removed'
      })
    } catch (error: any) {
      res.status(400).json({ error: error.message })
    }
